...
{
	if (!underWater || waterTex) return tex.SampleLevel(SmpUnfiltered, coord, 0);
	...
	{
		float4 samp = tex.SampleLevel(SmpUnfiltered,	    // prevent sampling other side
			saturate(coord + float2(i * pixelX, i * pixelY)) * .999, 0);
...
float4 GetSampleWithWater(float2 coord)
{
	...
	float2 depthSample = WaterDepth.SampleLevel(SmpUnfiltered, coord, 0);

	float waterDepth = depthSample.r;
	float sceneDepth = SceneDepth.SampleLevel(SmpUnfiltered, coord, 0).r;
	...
	if (depthDist > 0)
	{
		...
		float2 watercoord = waterize(coord, 1, 1, 1, saturate(waterDepth / fullDepth * (underWater ? .2 : 1)) * saturate(staticDistance / waterDist) * 16.f / 128.f);
...
{
	int2 c = dispatchThreadID.xy;
	float2 coord = float2(c) / Resolution;

	if (abs(CamPitch) >= 90) coord = -coord; 
	float4 trans = Transparency.SampleLevel(SmpFiltered, coord, 0);
